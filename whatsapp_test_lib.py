...
from whatsoup import user_is_logged_in
...
def scrape_is_exported(selected_chat, scraped):
    ...
    while not is_exported:
        export_txt(selected_chat, scraped)
        is_exported = True
...
def main():
    ...
    for c in range(1, len(chats)+1):
        #print(chats[c-1]['name'])

        #these to go in a loop
        selected_chat = chats[c-1]['name']  #'+256 775 844228'

        find_selected_chat(driver, selected_chat)

        load_selected_chat(driver)
